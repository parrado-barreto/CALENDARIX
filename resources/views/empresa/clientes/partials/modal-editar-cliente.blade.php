<div id="modalEditarCliente{{ $cliente->id }}" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
        <h2 class="text-lg font-bold text-purple-700 mb-4">✏️ Editar Cliente</h2>
        ...
        <form action="{{ route('empresa.clientes.update', ['empresa' => $empresa->id, 'cliente' => $cliente->id]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre" value="{{ $cliente->nombre }}" class="w-full border rounded px-3 py-2 mt-1" required>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Correo electrónico</label>
                <input type="email" name="email" value="{{ $cliente->email }}" class="w-full border rounded px-3 py-2 mt-1">
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Teléfono</label>
                <input type="text" name="telefono" value="{{ $cliente->telefono }}" class="w-full border rounded px-3 py-2 mt-1">
            </div>
